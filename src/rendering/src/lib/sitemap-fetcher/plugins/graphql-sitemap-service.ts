...
class GraphqlSitemapServicePlugin implements SitemapFetcherPlugin {
  ...
  async exec(context?: GetStaticPathsContext): Promise<StaticPath[]> {
    ...
    return process.env.EXPORT_MODE ? this._graphqlSitemapService.fetchExportSitemap(config.defaultLanguage) : this._graphqlSitemapService.fetchSSGSitemap(context?.locales || []);
  }
}
