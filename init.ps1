...
try {
    ...
    & $mkcert -install
    & $mkcert "*.camp-compassion.localhost"
...
Add-HostsEntry "cm.camp-compassion.localhost"
if ($Topology -ne "xp0") {
  Add-HostsEntry "cd.camp-compassion.localhost"
}
Add-HostsEntry "id.camp-compassion.localhost"
Add-HostsEntry "www.camp-compassion.localhost"
...
if ($InitEnv) {
    ...
    if ((Test-Path "$workingDirectoryPath\.env") -eq $false) {
    ...
    # CM_HOST
    Set-EnvFileVariable "CM_HOST" -Value "cm.camp-compassion.localhost"

    if ($Topology -ne "xp0") {
      # CD_HOST
      Set-EnvFileVariable "CD_HOST" -Value "cd.camp-compassion.localhost"
    }

    # ID_HOST
    Set-EnvFileVariable "ID_HOST" -Value "id.camp-compassion.localhost"

    # RENDERING_HOST
    Set-EnvFileVariable "RENDERING_HOST" -Value "www.camp-compassion.localhost"
